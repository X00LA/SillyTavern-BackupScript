...
files_to_backup=("data/*" "public/whitelist.txt" "config.yaml" "Backup_Linux.sh" "start.sh")
...
if command -v 7z &> /dev/null; then
    ...
    backup_file_name="SillyTavern_Backup_$dt.tar.gz"
    backup_file_path="$backup_path/$backup_file_name"
    tar -czf "$backup_file_path" "${files_to_backup[@]}"
fi